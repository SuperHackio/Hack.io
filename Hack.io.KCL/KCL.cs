using OpenTK;
using static Hack.io.Util.MathEx;
using Hack.io.BCSV;
using System;
...
namespace Hack.io.KCL
{
    ...
    public class KCL
    {
        ...
        private void Read(Stream KCLFile, BackgroundWorker bgw)
        {
            ...
            int PositionsOffset = BitConverter.ToInt32(KCLFile.ReadReverse(0, 4), 0);
            ...
            MaskZ = BitConverter.ToUInt32(KCLFile.ReadReverse(0, 4), 0);
            ShiftX = BitConverter.ToUInt32(KCLFile.ReadReverse(0, 4), 0);
            ShiftY = BitConverter.ToUInt32(KCLFile.ReadReverse(0, 4), 0);
            ShiftZ = BitConverter.ToUInt32(KCLFile.ReadReverse(0, 4), 0);
            ...
            {
                Positions.Add(new Vector3(BitConverter.ToSingle(KCLFile.ReadReverse(0, 4), 0), BitConverter.ToSingle(KCLFile.ReadReverse(0, 4), 0), BitConverter.ToSingle(KCLFile.ReadReverse(0, 4), 0)));
            ...
            {
                Normals.Add(new Vector3(BitConverter.ToSingle(KCLFile.ReadReverse(0, 4), 0), BitConverter.ToSingle(KCLFile.ReadReverse(0, 4), 0), BitConverter.ToSingle(KCLFile.ReadReverse(0, 4), 0)));
        ...
        private void Write(Stream KCLFile, BackgroundWorker bgw)
        {
            ...
            long FileStart = KCLFile.Position;
            //Header
            KCLFile.WriteReverse(BitConverter.GetBytes((int)0x38), 0, 4);
            KCLFile.WriteReverse(BitConverter.GetBytes(0x38 + (Positions.Count*12)), 0, 4);
            KCLFile.WriteReverse(BitConverter.GetBytes((0x38 + (Positions.Count * 12) + (Normals.Count * 12)) - 0x10), 0, 4);
            int OctreeOffset = (0x38 + (Positions.Count * 12) + (Normals.Count * 12)) + (Triangles.Count * 0x10);
            KCLFile.WriteReverse(BitConverter.GetBytes(OctreeOffset), 0, 4);

            KCLFile.WriteReverse(BitConverter.GetBytes(40f), 0, 4);
            KCLFile.WriteReverse(BitConverter.GetBytes(MinCoords.X), 0, 4);
            KCLFile.WriteReverse(BitConverter.GetBytes(MinCoords.Y), 0, 4);
            KCLFile.WriteReverse(BitConverter.GetBytes(MinCoords.Z), 0, 4);
            KCLFile.WriteReverse(BitConverter.GetBytes(MaskX), 0, 4);
            KCLFile.WriteReverse(BitConverter.GetBytes(MaskY), 0, 4);
            KCLFile.WriteReverse(BitConverter.GetBytes(MaskZ), 0, 4);
            KCLFile.WriteReverse(BitConverter.GetBytes(ShiftX), 0, 4);
            KCLFile.WriteReverse(BitConverter.GetBytes(ShiftY), 0, 4);
            KCLFile.WriteReverse(BitConverter.GetBytes(ShiftZ), 0, 4);

            for (int i = 0; i < Positions.Count; i++)
            {
                KCLFile.WriteReverse(BitConverter.GetBytes(Positions[i].X), 0, 4);
                KCLFile.WriteReverse(BitConverter.GetBytes(Positions[i].Y), 0, 4);
                KCLFile.WriteReverse(BitConverter.GetBytes(Positions[i].Z), 0, 4);
                bgw?.ReportProgress((int)GetPercentOf(i, MaxItemsForPogress), 2);
            }
            for (int i = 0; i < Normals.Count; i++)
            {
                KCLFile.WriteReverse(BitConverter.GetBytes(Normals[i].X), 0, 4);
                KCLFile.WriteReverse(BitConverter.GetBytes(Normals[i].Y), 0, 4);
                KCLFile.WriteReverse(BitConverter.GetBytes(Normals[i].Z), 0, 4);
            ...
            while (queuedNodes.Count > 0)
            {
                ...
                foreach (Octree node in nodes)
                {
                    ...
                    KCLFile.WriteReverse(BitConverter.GetBytes(node.Key), 0, 4);
            ...
            foreach (var ind in indexPool)
            {
                ...
                for (int i = 0; i < ind.Key.Length; i++)
                    KCLFile.WriteReverse(BitConverter.GetBytes((ushort)(ind.Key[i] + 1)), 0, 2); //-1 indexed
        ...
        public class KCLFace
        {
            ...
            {
                KCLFile.WriteReverse(BitConverter.GetBytes(Length), 0, 4);
                KCLFile.WriteReverse(BitConverter.GetBytes(PositionIndex), 0, 2);
                KCLFile.WriteReverse(BitConverter.GetBytes(DirectionIndex), 0, 2);
                KCLFile.WriteReverse(BitConverter.GetBytes(NormalAIndex), 0, 2);
                KCLFile.WriteReverse(BitConverter.GetBytes(NormalBIndex), 0, 2);
                KCLFile.WriteReverse(BitConverter.GetBytes(NormalCIndex), 0, 2);
                KCLFile.WriteReverse(BitConverter.GetBytes(GroupIndex), 0, 2);
            }
        }
        ...
        public class Octree : IEnumerable<Octree>
        {
            ...
            {
                Key = BitConverter.ToUInt32(KCLFile.ReadReverse(0, 4), 0);
                int terminator = 0x00;
                ...
                if ((Key >> 31) == 1) //Check for leaf
                {
                    ...
                    KCLFile.Position = offset + sizeof(ushort);
                        ...
                        ushort index;
                        while ((index = BitConverter.ToUInt16(KCLFile.ReadReverse(0, 2), 0)) != terminator)
                        {
            ...
            /// </summary>
            public Octree[] Children { get; internal set; }
        ...
        public class PaEntry : BCSVEntry
        {
            ...
            public int CameraID
            {
                get => (int)Data[BCSV.BCSV.FieldNameToHash(CAMERA_ID)];
                set => Data[BCSV.BCSV.FieldNameToHash(CAMERA_ID)] = (int)(value & (CAMERA_ID_MASK >> CAMERA_ID_SHIFT));
            }
            public int SoundCode
            {
                get => (int)Data[BCSV.BCSV.FieldNameToHash(SOUND_CODE)];
                set => Data[BCSV.BCSV.FieldNameToHash(SOUND_CODE)] = (int)(value & (SOUND_CODE_MASK >> SOUND_CODE_SHIFT));
            }
            public int FloorCode
            {
                get => (int)Data[BCSV.BCSV.FieldNameToHash(FLOOR_CODE)];
                set => Data[BCSV.BCSV.FieldNameToHash(FLOOR_CODE)] = (int)(value & (FLOOR_CODE_MASK >> FLOOR_CODE_SHIFT));
            }
            public int WallCode
            {
                get => (int)Data[BCSV.BCSV.FieldNameToHash(WALL_CODE)];
                set => Data[BCSV.BCSV.FieldNameToHash(WALL_CODE)] = (int)(value & (WALL_CODE_MASK >> WALL_CODE_SHIFT));
            }
            public int CameraThrough
            {
                get => (int)Data[BCSV.BCSV.FieldNameToHash(CAMERA_THROUGH)];
                set => Data[BCSV.BCSV.FieldNameToHash(CAMERA_THROUGH)] = (int)(value & (CAMERA_THROUGH_MASK >> WALL_CODE_SHIFT));
            }
            ...
            public PaEntry() { Init(); }
            public PaEntry(Dictionary<uint, BCSVField> FieldSource) : base(FieldSource) => Init();
            ...
            public static BCSV.BCSV CreateBCSV(WavefrontObj obj)
            {
                ...
                {
                    bcsv.Add(new PaEntry(bcsv.Fields));
                }
