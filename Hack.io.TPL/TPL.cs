...
namespace Hack.io.TPL
{
    ...
    {        
        /// <summary>
        /// Filename of this <see cref="TPL"/> file.
        /// </summary>
        public string FileName { get; set; } = null;
        public List<TPLTexture> Images = new List<TPLTexture>();
        private readonly byte[] Magic = new byte[4] { 0x00, 0x20, 0xAF, 0x30 };

        #region Auto Properties
        /// <summary>
        /// The Amount of images inside this TPL
        /// </summary>
        public int ImageCount => Images == null ? -1 : Images.Count;
        public TPLTexture this[int Index]
        {
            get => Images[Index];
            set
            {
        ...
        internal TPL() { }
        public TPL(string filename)
        {
            FileStream TPLFile = new FileStream(filename, FileMode.Open);
            Read(TPLFile);
            TPLFile.Close();
            FileName = filename;
        }
        public TPL(Stream memorystream) => Read(memorystream);
        ~TPL()
        {
            Console.WriteLine($"TPL \"{FileName ?? "???"}\" Disposed");
            for (int i = 0; i < ImageCount; i++)
                for (int x = 0; x < Images[i].ImageCount; x++)
                    Images[i][x].Dispose();
        }
        ...
        private void Read(Stream TPLFile)
        {
            ...
            for (int i = 0; i < ImageHeaderOffset.Count; i++)
            {
                ...
                TPLTexture current = new TPLTexture() { Format = Format, PaletteFormat = PaletteFormat, LODBias = LODBias, MagnificationFilter = (GXFilterMode)MaxFilter, MinificationFilter = (GXFilterMode)MinFilter, WrapS = (GXWrapMode)WrapS, WrapT = (GXWrapMode)WrapT, EnableEdgeLoD = EnableEdgeLOD };

                TPLFile.Position = ImageDataAddress;
                ushort ogwidth = ImageWidth, ogheight = ImageHeight;
                for (int x = 0; x <= MaxLOD; x++)
                {
                    current[x] = DecodeImage(TPLFile, PaletteData, Format, PaletteFormat, PaletteCount, ImageWidth, ImageHeight, x);
                }
                Images.Add(current);
            }
        }

        private void Write(Stream TPLFile)
        {
            long HeaderStart = TPLFile.Position;
            TPLFile.Write(Magic, 0, Magic.Length);

            TPLFile.WriteReverse(BitConverter.GetBytes(Images.Count), 0, 4);
            ...
            TPLFile.Write(new byte[Images.Count * 8], 0, Images.Count * 8);
            ...
            for (int i = 0; i < ImageCount; i++)
            {
                bool IsPalette = Images[i].Format.IsPaletteFormat();
                int PaletteHeader = !IsPalette ? 0 : (int)(TPLFile.Position - HeaderStart);
                List<byte> ImageData = new List<byte>();
                List<byte> PaletteData = new List<byte>();
                List<Bitmap> mips = Images[i].GetMipmaps();
                GetImageAndPaletteData(ref ImageData, ref PaletteData, mips, Images[i].Format, Images[i].PaletteFormat);
                if (IsPalette)
                {
                    TPLFile.WriteReverse(BitConverter.GetBytes((ushort)(PaletteData.Count / 2)), 0, 2);
                    TPLFile.WriteByte(0x00);
                    TPLFile.WriteByte(0x00);
                    TPLFile.WriteReverse(BitConverter.GetBytes((int)Images[i].PaletteFormat), 0, 4);
                    ...
                    TPLFile.WriteReverse(BitConverter.GetBytes((int)(temp2 - HeaderStart) + 4), 0, 4);
                ...
                TPLFile.WriteReverse(BitConverter.GetBytes((ushort)Images[i][0].Height), 0, 2);
                TPLFile.WriteReverse(BitConverter.GetBytes((ushort)Images[i][0].Width), 0, 2);
                TPLFile.WriteReverse(BitConverter.GetBytes((uint)Images[i].Format), 0, 4);
                long comebackhere = TPLFile.Position;
                TPLFile.WriteReverse(BitConverter.GetBytes(-572662307), 0, 4);
                TPLFile.WriteReverse(BitConverter.GetBytes((uint)Images[i].WrapS), 0, 4);
                TPLFile.WriteReverse(BitConverter.GetBytes((uint)Images[i].WrapT), 0, 4);
                TPLFile.WriteReverse(BitConverter.GetBytes((uint)Images[i].MinificationFilter), 0, 4);
                TPLFile.WriteReverse(BitConverter.GetBytes((uint)Images[i].MagnificationFilter), 0, 4);
                TPLFile.WriteReverse(BitConverter.GetBytes(Images[i].LODBias), 0, 4);
                TPLFile.WriteByte((byte)(Images[i].EnableEdgeLoD ? 0x01 : 0x00));
                TPLFile.WriteByte(0x00);
